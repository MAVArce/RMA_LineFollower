...
void ColorSearch::_calibrateGreen(){
    ...
    _greenFilter.hueMin = (hmin_high + hmin_low) / 2; 
    _greenFilter.hueMax = (hmax_high + hmax_low) / 2;
    _greenFilter.satMin = (smin_high + smin_low) / 2; 
    _greenFilter.satMax = (smax_high + smax_low) / 2;
}
...
void ColorSearch::_calibrateRed(){
    ...
    _redFilterBot.hueMax = (hmaxBot_high + hmaxBot_low) / 2;
    _redFilterBot.satMin = (sminBot_high + sminBot_low) / 2; 
    _redFilterBot.satMax = (smaxBot_high + smaxBot_low) / 2;
    ...
    _redFilterTop.hueMin = (hminTop_high + hminTop_low) / 2;
    _redFilterTop.satMin = (sminTop_high + sminTop_low) / 2; 
    _redFilterTop.satMax = (smaxTop_high + smaxTop_low) / 2;
...
void ColorSearch::FindLandmark(cv::Point *pt, int *dist, cv::Mat *image){
    ...
    *dist = -1;
    ...
    std::vector<std::vector<cv::Point>> greenContours = detect(_greenFilter);
    std::vector<std::vector<cv::Point>> redContours = detect(_redFilterBot, _redFilterTop, 2);
    ...
    cv::Moments redM2 = cv::moments(redContours[1],true);
    cv::Point redCenter1(redM1.m10/redM1.m00, redM1.m01/redM1.m00);
    cv::Point redCenter2(redM2.m10/redM2.m00, redM2.m01/redM2.m00);

    cv::Point middlePoint((redCenter1.x + redCenter2.x)/2, (redCenter1.y + redCenter2.y)/2);
